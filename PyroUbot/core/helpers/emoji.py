...
class EMO:
    ...
    async def PROSES(client):
        ...
        emot_prs = emot_4 if emot_4 else "5080331039922980916"
    ...
    async def BERHASIL(client):
        ...
        emot_brhsl = emot_5 if emot_5 else "5080277662069425163"
        if client.me.is_premium:
    ...
    async def GAGAL(client):
        ...
        emot_ggl = emot_6 if emot_6 else "5080555550748444526"
        if client.me.is_premium:
    ...
    async def BROADCAST(client):
        ...
        emot_gcs = emot_7 if emot_7 else "5080331039922980916"
        if client.me.is_premium:
    ...
    async def PASIR(client):
        ...
        emot_psr = emot_12 if emot_12 else "5080331039922980916"
        if client.me.is_premium:
            _pasir = f"<emoji id={emot_psr}>⚡️</emoji> "
        else:
            _pasir = ""
        return _pasir

    async def PREMIUM(client):
        emot_13 = await get_vars(client.me.id, "EMOJI_PREMIUM")
        emot_prem = emot_13 if emot_13 else "5080331039922980916"
        if client.me.is_premium:
            _premium = f"<emoji id={emot_prem}>⚡️</emoji>"
        else:
            _premium = ""
        return _premium
